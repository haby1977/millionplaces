...
// ============================================
// 🆕 CALCUL DU RATIO DE COULEUR SELON LE NOMBRE DE PHOTOS
// ============================================
function calculateColorRatio(totalPhotos) {
  // Paliers de progression lente vers la couleur
  if (totalPhotos < 500) return 0;                           // 0-499 : N&B pur (0%)
  if (totalPhotos < 1000) return 0.05;                       // 500-999 : 5% couleur
  if (totalPhotos < 1500) return 0.15;                       // 1000-1499 : 15% couleur
  if (totalPhotos < 2000) return 0.20;                       // 1500-1999 : 20% couleur
  if (totalPhotos < 2500) return 0.40;                       // 2000-2499 : 40% couleur
  if (totalPhotos < 3000) return 0.60;                       // 2500-2999 : 60% couleur
  if (totalPhotos < 3500) return 0.80;                       // 3000-3499 : 80% couleur
  return 1.0;                                                // 3500+ : couleur complète (100%)
}


// ============================================
// IMAGE OPTIMIZATION / CROP CARRÉ CENTRÉ + N&B + GRAIN + 🆕 DÉGRADÉ COULEUR
// ============================================
function optimizeAndPreview(file, colorRatio, cb) {
  ...
  reader.onload = e => {
    ...
    img.onload = () => {
      ...
      // 🆕 CONVERSION N&B AVEC DÉGRADÉ VERS LA COULEUR
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // Calcul de la luminosité (pour le N&B)
        const gray = r * 0.299 + g * 0.587 + b * 0.114
        
        // Interpolation entre N&B (gray) et couleur (r, g, b)
        data[i] = gray + (r - gray) * colorRatio       // R
        data[i + 1] = gray + (g - gray) * colorRatio   // G
        data[i + 2] = gray + (b - gray) * colorRatio   // B
      }
...
// ============================================
// 🆕 GESTIONNAIRE INPUT FILE AVEC CALCUL DU RATIO
// ============================================
document.getElementById('photoUpload')?.addEventListener('change', async e => {
  ...
  // 🆕 Récupérer le nombre total de photos pour calculer le ratio
  try {
    const { count } = await supabase
      .from('objets')
      .select('*', { count: 'exact', head: true })
    
    const colorRatio = calculateColorRatio(count || 0)
    
    optimizeAndPreview(file, colorRatio, (optFile, previewUrl) => {
      if (!optFile) return
