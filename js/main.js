...
// ============================================
// CONFIGURATION (SUPRESSION DES CLÉS INUTILES)
// ============================================
// ATTENTION : Les clés sont supprimées ici car elles ne seront plus utilisées par le front.
// C'est votre proxy Vercel qui utilise les clés.
...
// Suppression du client Supabase pour le frontend, il est désormais géré par le proxy
...
// ============================================
// SHUFFLE + LOAD GALLERY (CORRIGÉ: Utilisation du Proxy Vercel)
// ============================================
...
async function loadGallery() {
  try {
    // ? CORRECTION: Appel au proxy Vercel pour éviter le blocage CORS
    const response = await fetch('/api/supabase-proxy');

    if (!response.ok) {
        // Le proxy lui-même peut avoir échoué (erreur Vercel ou Supabase)
...
// ============================================
// SUBMIT (CORRIGÉ: Proxy pour vérification email)
// ============================================
...
document.getElementById('uploadForm')?.addEventListener('submit', async e => {
  ...
  // 1. VÉRIFICATION DE L'EMAIL (SUPPRIMÉ : APPEL DIRECT SUPABASE)
  // L'appel direct à Supabase pour la vérification de l'email est retiré
  // pour deux raisons : 1) CORS, 2) Risque de sécurité (énumération d'emails).
  // La vérification doit idéalement se faire côté serveur Vercel ou être ignorée ici.

  // ? SIMPLIFICATION : Nous laissons cette vérification au backend (create-checkout.js)
  // qui est déjà en place et sécurisé, évitant un appel API supplémentaire ici.
  ...
  try {
    ...
    // Appel à l'API Vercel /api/create-checkout (CORS est résolu)
 ...
 * @param {File} file L'objet File sélectionné.
 * @returns {Promise<string>} Promesse résolue avec la chaîne Base64 (incluant l'en-tête MIME).
 */
